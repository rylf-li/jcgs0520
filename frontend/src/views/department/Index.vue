  ...
  <div class="department-page">
    ...
    <div class="main-content">
      ...
      <div class="toolbar-section">
        <div class="toolbar-left">
          <div class="search-box">
            ...
            <el-input v-model="searchText" placeholder="搜索部门名称..." clearable @input="handleSearch" />
          </div>
        </div>
        <div class="toolbar-right">
          <ImportExportButtons model="departments" modelName="部门" @success="loadData" />
        ...
        >
          ...
          <el-table-column label="部门名称" min-width="180">
            <template #default="{ row }">
              <el-input v-if="row._editing" v-model="row.name" placeholder="部门名称" size="small" />
              <span v-else class="cell-name">{{ row.name }}</span>
            </template>
          </el-table-column>
          <el-table-column label="描述" min-width="220" show-overflow-tooltip>
            <template #default="{ row }">
              <el-input v-if="row._editing" v-model="row.description" placeholder="描述" size="small" />
              <span v-else class="cell-value text-muted">{{ row.description || '-' }}</span>
            </template>
          </el-table-column>
          <el-table-column label="员工数" width="100" align="center">
            <template #default="{ row }">
              <el-tag type="info" size="default" class="emp-tag">{{ row.employee_count || 0 }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="160" align="center" fixed="right">
            <template #default="{ row }">
              ...
              <template v-else>
                <el-button type="success" size="small" plain @click="$router.push(`/departments/${row.id}`)" class="btn-view">
      ...
      <div class="pagination-wrapper">
        <span class="total">共 <b>{{ total }}</b> 个部门</span>
        <el-pagination
    ...
    <!-- 新增/编辑弹窗 -->
    <el-dialog v-model="dialogVisible" :title="editId ? '编辑部门' : '新增部门'" width="520px" destroy-on-close class="edit-dialog">
...
const batchDelete = async () => {
  ...
  try { await ElMessageBox.confirm('确定删除选中的 ' + selectedRows.value.length + ' 条部门吗？', '批量删除', { type: 'warning' }) } catch { return }
  batchLoading.value = true
  try {
    let ok = 0
    for (const row of selectedRows.value) { try { const r = await departmentApi.delete(row.id); if (r.success) ok++ } catch {} }
...
const totalEmployees = computed(() => list.value.reduce((sum, r) => sum + (r.employee_count || 0), 0))
...
const saveRow = async (row) => {
  if (!row.name) { ElMessage.warning('部门名称不能为空'); return }
  // 二次确认
  try { await ElMessageBox.confirm('确定要保存修改吗？', '确认保存', { type: 'info' }) } catch { return }
  try {
    const data = { name: row.name, description: row.description }
    const res = await departmentApi.update(row.id, data)
...
const handleSave = async () => {
  ...
  try { await ElMessageBox.confirm(`确定要${action}该部门吗？`, '确认保存', { type: 'info' }) } catch { return }
  saving.value = true
  try {
...
.emp-tag { background: #f0f2f7; border: none; color: #4a4a5a; font-weight: 500; }
