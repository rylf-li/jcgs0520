    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span class="card-title">季度报表</span>
          <div class="filter-bar">
            <el-select v-model="query.year" style="width:100px" @change="loadData">
              <el-option v-for="y in years" :key="y" :label="`${y}年`" :value="y" />
            </el-select>
            <el-select v-model="query.quarter" style="width:100px;margin-left:8px" @change="loadData">
              <el-option label="第1季度" :value="1" /><el-option label="第2季度" :value="2" />
              <el-option label="第3季度" :value="3" /><el-option label="第4季度" :value="4" />
            </el-select>
          </div>
        </div>
      </template>
      ...
      <div class="summary-bar">
        <span>统计周期：{{ data.startDate }} 至 {{ data.endDate }}</span>
        <span style="margin-left:16px">汇总口径：{{ data.statsSource === 'snapshot' ? '快照统计' : '实时统计' }}</span>
      </div>
    </el-card>
  </div>
</template>
...
const query = reactive({ year: now.year(), quarter: Math.ceil((now.month() + 1) / 3) })
const data = ref({ financeList: [], companyStats: [], deptStats: [], startDate: '', endDate: '' })

const fmtMoney = (v) => v ? Number(v).toLocaleString('zh-CN', { minimumFractionDigits: 2 }) : '0.00'

const loadData = async () => {
  const res = await reportApi.quarterly(query)
