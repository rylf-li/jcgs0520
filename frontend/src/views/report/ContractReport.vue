<template>
  <div class="page-container">
    <el-card shadow="never">
      <template #header><span class="card-title">合同报表</span></template>
      <el-form inline style="margin-bottom:16px">
        ...
        <el-form-item label="合同名称"><el-input v-model="query.name" placeholder="请输入" clearable style="width:140px" /></el-form-item>
      ...
      <el-table :data="list" stripe border v-loading="loading" max-height="500">
        <el-table-column prop="contract_no" label="合同编号" width="150" fixed />
        <el-table-column prop="date" label="日期" width="100" />
        <el-table-column prop="name" label="合同名称" min-width="160" show-overflow-tooltip fixed />
        ...
        <el-table-column prop="amount" label="合同金额" width="110" align="right"><template #default="{ row }">{{ fmtMoney(row.amount) }}</template></el-table-column>
        ...
        <el-table-column prop="status" label="状态" width="90">
          <template #default="{ row }"><el-tag :type="{ '待签订': 'warning', '进行中': 'primary', '已完成': 'success', '已取消': 'danger' }[row.status]" size="small">{{ row.status }}</el-tag></template>
        </el-table-column>
      </el-table>
...
const total = ref(0)
const dateRange = ref([])
const query = reactive({ page: 1, pageSize: 20, contract_no: '', name: '', customer_name: '', status: '', date_start: '', date_end: '' })
...
const loadData = async () => {
  ...
  loading.value = true
  try {
    const res = await reportApi.contract(query)
    if (res.success) {
...
const resetQuery = () => {
  Object.assign(query, { page: 1, contract_no: '', name: '', customer_name: '', status: '', date_start: '', date_end: '' })
  dateRange.value = []
  loadData()
}
